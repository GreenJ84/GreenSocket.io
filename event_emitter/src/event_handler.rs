...
pub trait EventHandler<T: Send + Sync>: Send + Sync {
    ...
    fn emit_async(&mut self, event_name: &str, payload: EventPayload<T>) -> Box<dyn Future<Output=()> + Send>;
    /// Fire and forget all event callbacks before deleting the event
    fn emit_final_async(&mut self, event_name: &str, payload: EventPayload<T>) -> Box<dyn Future<Output=()> + Send>;
}